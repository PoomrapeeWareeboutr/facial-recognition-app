...
<template>
  <div class="max-w-[50rem] flex flex-col mx-auto w-full h-full">
    <main id="content" role="main">
      <div class="text-center max-w-[85rem] mx-auto px-4 sm:px-6 lg:px-8 py-10 lg:py-16">
        <h1 class="block font-semibold text-gray-800 text-4xl md:text-5xl lg:text-6xl dark:text-gray-200">
          You're in, 
          <span class="bg-clip-text bg-gradient-to-l from-blue-600 to-violet-500 text-transparent dark:from-blue-400 dark:to-violet-400">
            {{ username }}!
          </span>
          🎉
        </h1>
        <p class="mt-3 text-lg text-gray-600 dark:text-gray-400">
          This route is restricted to authorized users only, and you are one of them! 
          Congratulations on gaining access.
        </p>
        <div class="mt-5 flex flex-col justify-center items-center gap-2 sm:flex-row sm:gap-3">
          <router-link :to="{ name: 'SignIn' }" @click="signout" class="w-full sm:w-auto inline-flex justify-center items-center gap-x-3 text-center bg-blue-600 hover:bg-blue-700 border border-transparent text-white text-sm font-medium rounded-md focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-offset-2 focus:ring-offset-white transition py-3 px-4 dark:focus:ring-offset-gray-800">
