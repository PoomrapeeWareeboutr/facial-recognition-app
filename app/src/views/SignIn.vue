...
<template>
  <div class="w-full max-w-md mx-auto p-6">
    <Alert v-if="redirectPath !== '/'" :title="`Unable to access '${redirectPath}'`">
      ...
      <div class="p-4 sm:p-7">
        <div class="text-center">
          ...
          <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
            ...
            <router-link :to="{ name: 'SignUp' }" class="text-blue-600 decoration-2 hover:underline font-medium" href="../examples/html/signup.html">
              Sign up here
            </router-link>
          </p>
        </div>
        <div class="mt-5">
          ...
          <form>
            <div class="grid gap-y-4">
              ...
              <!-- Form Group -->
              <div>
                <label for="username" class="block text-sm mb-2 dark:text-white">Face ID</label>
                <div class="relative">
              ...
              <p v-if="unauthorized" class="text-xs text-red-600 mt-2">Your username does not exist or your Face ID does not match.</p>
              <!-- End Form Group -->
              <button :disabled="username.length === 0 ? true : false" @click.prevent="signin" type="submit" class="py-3 px-4 inline-flex justify-center items-center gap-2 rounded-md border border-transparent font-semibold bg-blue-500 text-white hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-all text-sm dark:focus:ring-offset-gray-800">Sign in</button>
            </div>
          </form>
          <!-- End Form -->
        </div>
